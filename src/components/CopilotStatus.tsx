...
const CopilotStatus = ({ agentId }: { agentId: string }) => {
  ...
  if (!copilotkit || !agent) {
    return (
      <div
        className={`absolute top-4 right-4 flex items-center space-x-2 p-2 rounded-lg shadow-lg bg-red-500`}
      >
  ...
  return (
    <div
      className={`absolute top-4 right-4 flex items-center space-x-2 p-2 rounded-lg shadow-lg ${getStatusColor(copilotkit?.runtimeConnectionStatus ?? '')}`}
    >
      <span className="text-white">Runtime: {copilotkit.runtimeConnectionStatus}</span>
      {copilotkit.runtimeVersion && (
        <span className="text-white">v{copilotkit.runtimeVersion}</span>
      )}
